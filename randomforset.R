...
train<-data[ind==1,]
test<-data[ind==2,]
randomforestmodel<-randomForest(NSP~.,data=train)
randomforestmodel
p1<-predict(randomforestmodel,train)
tab<-table(p1,train$NSP)
tab
matrix<-table(p1,train$NSP)
matrix
a<-matrix[1,1]
b<-matrix[1,2] 
c<-matrix[1,3]
...
fmeasureC
p11<-predict(randomforestmodel,test)
matrix<-table(p11,test$NSP)
matrix
a<-matrix[1,1]
b<-matrix[1,2]
c<-matrix[1,3]
